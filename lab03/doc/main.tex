...
A Verilog simulator, such as Icarus Verilog\cite{VERILOG}, reduced testing
times although it was not as strict as when the code is actually synthesized.
...
% HDL programming
The HDL code used to program the CPLD was written in Verilog.
...
It was often trivial to devise a solution but the code had to be significantly
modified in order to get it to synthesize.
For example, suppose it was desired to increment a variable on the rising
edge and falling edge of a clock.
The Verilog simulator (Icarus)  will allow this but it wont synthesize
because it cannot be turned in to a flip flop.
As another example, suppose it was desired to reset all the variables on
a starting of NSS, and then during SCK edges modify these variables.
Again this will be allowed in the simulator but it is not synthesizeable
because two different always blocks cannot modify the same variable.

And the information on synthesis errors is often obscure.
In some cases all that is provided is a code and a terse message.
Using the Synplify tool a more detailed description can be found.
And they provide examples of code which causes the error along with
how the code can be changed to eliminate the error.
But the behavior of changed code is often not functionally equivalent.

% was it a success?
While this project was a success in developing bus communications over SPI
